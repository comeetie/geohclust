...
```{r}
idf_agg=geocutree(res,k=800) |> rowwise(cl) |> mutate(total = sum(c_across(nodep:tcom)))
...
map2=ggplot(idf_agg)+geom_sf(aes(fill=tcom/total),size=0.2)+theme_bw()+scale_fill_distiller("Gauche :",palette = "Reds",direction = 1,limits=c(0,0.75))
